...
export default function shounenAi() {

    const [anime,setAnime] = useState([])
    const [actualPage,setActualPage] = useState(1)

    const apiUrl = `https://api.jikan.moe/v3/search/anime?q=&page=${actualPage}&genre=28/`
    ...
    return (
        <Div>
            <Head title="animes shounen ai"/>
            <header>
                <Div bgImage={BannerImage} width="100%">
                   <Navbar/>
                    <Banner mainText="Você está na sessão animes shounen ai"/>
                </Div>
            </header>
            <main>
